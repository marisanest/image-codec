...
class EntropyDecoder:
    ...
    def decode_block(self, block: Block) -> PartitioningModeParameters:
        partitioning_mode_parameters = PartitioningModeParameters(partitioning_mode=self.decode_partitioning_mode())

        for partition in block.partitions(partitioning_mode_parameters.partitioning_mode):
            prediction_mode_parameters = PredictionModeParameters(prediction_mode=self.decode_prediction_mode())
    ...
    def decode_q_indexes(self, block_size: int) -> np.array:
        ...
        if not self.arithmetic_decoder.decode_bit(self.context_modeler.probability_codeblock_flag):
            return q_indexes_block.reshape([block_size, block_size]).reshape(block_size, block_size)

        last_scan_index = self.exp_golomb_prob_adapted(self.context_modeler.probability_last_prefix)
        assert (last_scan_index >= 0)
        assert (last_scan_index < int(block_size * block_size))
        
        q_indexes_block[last_scan_index] = self.decode_q_index(last_scan_index, is_last=True)
    ...
    def decode_q_index(self, index: int, is_last=False) -> int:
        ...
        if not is_last:
            if self.arithmetic_decoder.decode_bit(self.context_modeler.probability_signal_flag) == 0:
                return 0

        if self.arithmetic_decoder.decode_bit(self.context_modeler.probability_gt1_flag) == 0:
            return sign(self.arithmetic_decoder.decode_bit_bypass())

        q_index = self.exp_golomb_prob_adapted(self.context_modeler.probability_level_prefix) + 2
        q_index *= sign(self.arithmetic_decoder.decode_bit_bypass())

        return q_index

    def decode_partitioning_mode(self) -> PartitioningMode:
        return PartitioningMode.NON_SUB_PARTITIONING if self.arithmetic_decoder.decode_bit(self.context_modeler.partitioning_mode_bit) == 0 else PartitioningMode.SUB_PARTITIONING

    def decode_prediction_mode(self) -> PredictionMode:
        if self.arithmetic_decoder.decode_bit(self.context_modeler.prediction_mode_bit1) == 0:
            return PredictionMode.PLANAR_PREDICTION
        elif self.arithmetic_decoder.decode_bit(self.context_modeler.prediction_mode_bit2) == 0:
            return PredictionMode.DC_PREDICTION
        elif self.arithmetic_decoder.decode_bit(self.context_modeler.prediction_mode_bit3) == 0:
