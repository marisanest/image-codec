...
class Block:
    def __init__(
        ...
        prediction_error: np.array = None,
        q_indexes: np.array = None,
    ...
    def encode(
        ...
        prediction_calculator: "PredictionCalculator.PredictionCalculator",
        qs: float,
        ...
        self.q_indexes = (
            ...
            * np.floor((np.abs(transform_coefficients) / qs) + 0.4)
        ).astype("int")
    ...
    def predict(
        ...
        self.prediction = (
            prediction_calculator.get_inter_prediction(self, mx, my)
            if inter
            else prediction_calculator.get_prediction(self, prediction_mode)
        )

    def reconstruct(
        self, prediction_mode: PredictionMode, qs: float, transformer: Transformer
    ):
    ...
    def sort_q_indexes(self, prediction_mode: PredictionMode, decode=False):
        if (
            ...
            or prediction_mode == PredictionMode.PLANAR_PREDICTION
        ):
            self.q_indexes = (
                invert_diagonal_sort(self.q_indexes)
                if decode
                else sort_diagonal(self.q_indexes)
            )
    ...
    def estimate_bit_rate(
        ...
        prediction_mode: PredictionMode,
        entropy_encoder: "EntropyEncoder.EntropyEncoder",
